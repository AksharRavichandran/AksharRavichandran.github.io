...
/**
 * Tabs + panel (framer-motion cross-fade). With `integratedCard`, tabs are plain text + bold
 * selection; otherwise a sliding pill highlights the active tab.
 * @param {{ id: string, label: string, content: React.ReactNode }[]} [tabs]
 * @param {string} [defaultTab]
 * @param {string} [className]
 * @param {string} [layoutIdPrefix] — unique per mount when multiple instances exist
 * @param {boolean} [integratedCard] — strip outer chrome when wrapped in `.home-hero-card`
 * @param {boolean} [large] — wider tabs + taller panel (About story)
 * @param {boolean} [fillHeight] — flex column: tab row + panel share one scroll area (tabs scroll away with content)
 * @param {string} [panelClassName] — merged onto the tab panel container
 */
